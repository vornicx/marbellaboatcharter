...
if (homeSelection) {
  const selectionIds = ['M08', 'M11', 'M15', 'S02', 'F02', 'G02'];
  const selected = selectionIds.map(id => fleet.find(v => v.id === id)).filter(Boolean);
  homeSelection.innerHTML = selected.map(vessel => `
    <a class="selector-card" href="${vesselHref(vessel)}" aria-label="${vessel.name}, ${vessel.pax} guests, from ${vessel.price}">
      <div class="selector-card__top"><span>${vessel.id}</span><span>${vessel.type}</span></div>
      <div class="selector-card__title">${vessel.name}</div>
      <p>${vessel.route}</p>
      ...
      <div class="selector-card__price">From ${vessel.price}</div>
    </a>
  `).join('');
}
  ...
  register.innerHTML = fleetGroups.map(group => {
    ...
    return `
      <section class="fleet-group" id="${group.key}" aria-labelledby="group-${group.key}">
        ...
        <div class="fleet-head-row" aria-hidden="true"><span>ID</span><span>Vessel</span><span>Pax</span><span>LOA</span><span>Use / route</span><span>From</span></div>
        ${vessels.map(v => `
...
qsa('[data-dispatch-form]').forEach(form => {
  form.addEventListener('submit', event => {
    ...
    const lines = [
      ...
      `Departure area: ${data.get('port') || 'Flexible'}`,
      `Plan / yacht: ${data.get('plan') || 'Open to recommendation'}`,
